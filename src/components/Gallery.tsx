      ...
      <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {events.map((ev, i) => (
          <Reveal key={ev.id} delay={i * 0.06}>
            <article
              onClick={() => open(ev)}
              className="group cursor-pointer overflow-hidden rounded-2xl border border-slate-200 bg-white/80 shadow-sm transition hover:shadow-md dark:border-slate-800 dark:bg-slate-900/70"
            >
              ...
              <div className="p-4">
                ...
                {ev.tags?.length ? (
                  <div className="mt-2 flex flex-wrap gap-2">
                    {ev.tags.map((t) => (
                      <span key={t} className="rounded-full border border-slate-200 px-2 py-0.5 text-xs text-slate-600 dark:border-slate-700 dark:text-slate-300">{t}</span>
                    ))}
                  </div>
                ) : null}
              </div>
        ...
        {active && (
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3">
              <button onClick={prev} className="rounded-xl border px-3 py-1 text-sm hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-800">Prev</button>
              <button onClick={next} className="rounded-xl border px-3 py-1 text-sm hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-800">Next</button>
